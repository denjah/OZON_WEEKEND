...
export default function App() {
  // Generate reproducible mock data instead of using Math.random() in render
  const generateMockData = () => {
    return Array.from({ length: 20 }, (_, i) => {
      // Pseudo-random deterministic values based on index
      const pseudoX = (i * 37) % 100;
      const pseudoY = (i * 73) % 100;
      const pseudoSize = ((i * 19) % 30) + 5;
      const pseudoCat = ['A', 'B', 'C', 'D'][i % 4];
      return {
        id: i,
        label: `Point ${i + 1}`,
        x: pseudoX,
